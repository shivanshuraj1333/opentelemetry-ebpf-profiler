...
// runs in a nested PID namespace. When profiler_pidns_level == 0 the helpers
// short-circuit to bpf_get_current_pid_tgid() >> 32.
//
// task_struct / struct pid / struct upid offsets are filled from BTF at
// load time by parseBTF(); profiler_pidns_level is discovered once at
...
// pidns_translation_available is true when parseBTF() populated the
...
bool pidns_translation_available(void)
{
  return task_thread_pid_offset != 0;
}

// current_task_tgid_at_level returns the TGID (i.e. what getpid(2) returns)
// of the on-CPU task as it appears in the PID namespace at depth `level`.
// Walks via task->group_leader->thread_pid so the result is the process
// TGID regardless of which thread of the process is currently on-CPU.
// Returns 0 for kernel threads, for tasks whose pidns is shallower than
// `level`, or on any read failure.
static inline __attribute__((__always_inline__))
u32 current_task_tgid_at_level(u32 level)
{
  ...
  u32 task_level;
  if (bpf_probe_read_kernel(&task_level, sizeof(task_level),
  ...
  u32 nr;
  if (bpf_probe_read_kernel(&nr, sizeof(nr),
                            pid + pid_numbers_offset + level * upid_size +
                              upid_nr_offset))
    return 0;
  return nr;
}
...
static inline __attribute__((__always_inline__))
bool is_our_analysis_task(u32 caller_pid)
{
  // When PID-ns translation isn't available (pre-4.19 / no BTF), preserve
  // the pre-fix kernel-root comparison. This means the analysis probes only
  // succeed if the profiler runs in the kernel-root pidns — same constraint
  // as before the fix.
  ...
  u32 nr;
  if (bpf_probe_read_kernel(&nr, sizeof(nr),
                            pid + pid_numbers_offset +
                              task_level * upid_size + upid_nr_offset))
    return false;
...
static inline __attribute__((__always_inline__))
u32 get_pid_in_profiler_ns(void)
{
